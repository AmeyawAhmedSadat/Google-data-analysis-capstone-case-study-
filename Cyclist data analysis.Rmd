...
```{r}
bike_rides_df$start_hour <- strptime(bike_rides_df$start_time, "%H") %>% 
        hour()
bike_rides_df$end_hour <- strptime(bike_rides_df$end_time, "%H") %>% 
        hour()
...
### view final dataframe before analysis
```{r}
...
bike_rides_df_new %>% 
        ...
        summarise(min_trip= min(trip_duration),
...
```{r}
bike_rides_df_new %>%
        select(customer_type, week_day, trip_duration) %>%
...
### Total number of trips by bike type and start hour
```{r}
bike_rides_df_new %>% 
        select(bike_type, trip_duration, start_hour, customer_type) %>% 
        group_by(bike_type, customer_type) %>% 
        summarise(average_trip_mins = mean(trip_duration),
                  number_of_trips =n (),
                  starting_trip_hour = mean(start_hour)) %>% 
        arrange(customer_type, desc(number_of_trips))
```
